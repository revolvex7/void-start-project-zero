...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
		...
		try {
			...
			if (response?.data) {
				...
				const updatedUser = {
					...
					mainGoal: Array.isArray(userData.mainGoal)
						? userData.mainGoal
						: userData.mainGoal
						? [userData.mainGoal]
						: [],
					...
					profileImage:
						userData.profileImage || user?.profileImage || user?.avatar || "",
	...
	const login = async (
		...
		try {
			...
			if (domain && domain.includes('.ilmee.ai')) {
				subdomain = domain.replace('.ilmee.ai', '');
			}
			
			console.log('🔍 Login domain processing:', { original: domain, subdomain });
			
			const loginData: LoginRequest = { emailOrUsername, password, domain: subdomain };
			...
			if (accessToken) {
				...
				const userDomain =
					response?.data?.data?.user?.domain || domain || "ilmee";
